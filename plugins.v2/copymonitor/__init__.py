...
class CopyMonitor(_PluginBase):
    ...
    # 作者主页
    author_url = "https://github.com/diabellstar"
    # 插件配置项ID前缀
    plugin_config_prefix = "diabellstar_copymonitor_"
    # 加载顺序
    plugin_order = 4
    # 可使用的用户级别
    auth_level = 1
        ...
        try:
            ...
            with lock:
                ...
                if self._size and float(self._size) > 0 and file_path.stat().st_size < float(self._size) * 1024:
                    logger.info(f"{event_path} 文件大小小于最小文件大小，忽略...")
                    # _transfer_type = "copy"
                    return
                else:
                    _transfer_type = "copy"
                ...
                if not state:
                    # 转移失败
                    logger.warn(f"{file_path.name} 复制失败：{errmsg}")
                    if self._notify:
                        self.post_message(
